...
// ====================== Khách Hàng =============================
// Load danh sách user cho admin. Nếu có Firebase thì lắng nghe realtime, nếu không thì dùng localStorage.
var adminUsersUnsubscribe = null;

function fetchUsersForAdmin(callback) {
    // Không có Firebase: dùng dữ liệu local hiện tại
    if (!window.firebaseDb) {
        callback(getListUser());
        return;
    }

    // Đã có listener thì chỉ cần gọi lại callback với dữ liệu local
    if (adminUsersUnsubscribe && window.__lastAdminUsersSnapshot) {
        callback(window.__lastAdminUsersSnapshot);
        return;
    }

    // Đăng ký lắng nghe realtime trên collection users
    try {
        adminUsersUnsubscribe = window.firebaseDb.collection('users').onSnapshot(function (snapshot) {
            var users = [];
            ...
            window.__lastAdminUsersSnapshot = users;
            // Đồng bộ về localStorage để các hàm khác (đơn hàng, thống kê) dùng chung dữ liệu
            try {
                setListUser(users);
            } catch (e) {
                console.error('Sync users snapshot to localStorage error:', e);
            }
            callback(users);
        }, function (err) {
            console.error('Firebase fetchUsersForAdmin onSnapshot error:', err);
            callback(getListUser());
        });
    } catch (e) {
        console.error('Firebase fetchUsersForAdmin error:', e);
        callback(getListUser());
    }
}
