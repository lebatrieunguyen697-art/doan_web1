...
function khoiTao() {
    ...
    // Đồng bộ danh sách user từ Firebase (nếu có) về localStorage để các thiết bị dùng chung dữ liệu
    syncUsersFromFirebaseOnStart();
...
// Đồng bộ ListUser từ Firebase về localStorage khi khởi tạo trang (nếu có Firebase)
function syncUsersFromFirebaseOnStart() {
    try {
        ...
        window.firebaseDb.collection('users').get()
            ...
            .catch(function (err) {
                console.error('syncUsersFromFirebaseOnStart error:', err);
            });
    } catch (e) {
        console.error('syncUsersFromFirebaseOnStart exception:', e);
    }
}
